...
public class RegisterCommand implements CommandExecutor {
    ...
    public RegisterCommand(CrystalAuth plugin){
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String s, String[] args) {
        ...
        if(accountsConfig.contains(uuid)){
            player.sendMessage(translate(config.getString("already-registered")));
            return true;
        }

        if(args.length != 1){
            player.sendMessage(translate(config.getString("register-usage")));
            return true;
        }

        accountsConfig.set(uuid + ".password", args[0]);
        plugin.getAccountManager().saveAccountsFile();

        player.sendMessage(translate(config.getString("successfully-registered")));
